...
RSpec.describe "as a merchant employee user" do
  ...
  it 'can see orders that have been place for that merchants products' do
    ...
    expect(page).to have_content("Order placed: ##{item_order.created_at.formatted_to_s(:long)}")
    ...
    expect(current_path).to eq("/merchant/orders/#{item_order.id}")

    expect(page).to have_link("#{item_order2.order_id}")
    expect(page).to have_content("Order##{item_order2.order_id}")
    expect(page).to have_content("Order placed: ##{item_order2.created_at.formatted_to_s(:long)}")
